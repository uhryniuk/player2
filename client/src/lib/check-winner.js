...
const checkHorizontal = (board, client) => {
	...
	for (let c = 0; c < NUM_COL - 3; c++) {
		...
		for (let r = 0; r < NUM_ROW; r++) {
			// each row
			for (let i = 0; i < 4; i++) {
				// recall you need 4 to win
				if (board[r][c + i] == client) {
					// if not all pieces match
...
const checkWinner = (board) => {
	const runCheckers = (player) => {
		const checks = checkHorizontal(board, player) || checkVertical(board, player) || checkDiagonal(board, player)
